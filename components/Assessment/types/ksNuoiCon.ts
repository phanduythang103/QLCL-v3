export interface KsNuoiConRecord {
  ...
  delivery_type?: number;     // Hình thức sinh: 1=Đẻ thường, 2=Mổ đẻ
  ...
  reason_no_consult?: number;     // Lý do không được tư vấn (1-5)
  cord_cut?: number;              // Cắt dây rốn (1-3)
  skin_to_skin?: number;          // Da kề da (1-2)
  first_breastfeed?: number;      // Thời điểm bú đầu tiên (1-8)
  support_person?: number[];      // Ai hỗ trợ bú (1-5)
  support_type?: number[];        // Hình thức hỗ trợ (1-5)
  other_food?: number;            // Cho ăn thêm (1-5)
  suggest_formula?: number;       // Gợi ý sữa công thức (1-2)
  benefits?: string;              // Lợi ích sữa mẹ (textarea)
  exclusive_months?: number;      // Bú hoàn toàn (tháng)
  total_months?: number;          // Tổng thời gian bú (tháng)
