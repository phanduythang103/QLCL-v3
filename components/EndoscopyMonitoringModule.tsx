...
import {
  fetchEndoscopyMonitoring, addEndoscopyMonitoring, updateEndoscopyMonitoring,
  deleteEndoscopyMonitoring
} from '../readEndoscopyMonitoring';
...
export const CRITERIA_ENDOSCOPY = [
  ...
  { id: 'a1_3', label: 'Chỉ định kỹ thuật: Nội soi đại trực tràng có gây mê' },
  { id: 'a1_4', label: 'Chống chỉ định' },
  { id: 'a1_5', label: 'Đã chuẩn bị đại trực tràng sạch' },
  { id: 'a1_6', label: 'Đã được nghe giải thích về lợi ích, tai biến của kỹ thuật và đồng ý can thiệp' },
  { id: 'a2_1', label: 'Phiếu khai thác tiền sử dị ứng' },
  { id: 'a2_2', label: '01 Bác sĩ chuyên khoa nội soi tiêu hóa' },
  { id: 'a2_3', label: '02 Điều dưỡng hoặc kỹ thuật viên phụ' },
  { id: 'a2_4', label: 'Trang phục phòng hộ đầy đủ, rửa tay đúng quy định' },
  { id: 'a3_1', label: 'Kiểm tra hệ thống nội soi tiêu hóa' },
  { id: 'a3_2', label: 'Kiểm tra dụng cụ sinh thiết' },
  { id: 'a4_1', label: 'Kiểm tra chỉ định, cam kết' },
  { group: 'B. CÁC BƯỚC TIẾN HÀNH' },
  { id: 'b5_1', label: 'Đo mạch, huyết áp' },
  { id: 'b5_2', label: 'Đặt dịch truyền TM, thở oxy, mắc monitor theo dõi' },
  { id: 'b5_3', label: 'Tư thế người bệnh: Nằm nghiêng trái' },
  { id: 'b5_4', label: 'Tiêm thuốc tiền mê' },
  { id: 'b5_5', label: 'Thăm hậu môn- trực tràng' },
  { id: 'b6_1', label: 'Bôi trơn đầu ống soi bằng gel KY' },
  { id: 'b6_2', label: 'Đánh giá mức độ làm sạch đại tràng theo thang điểm Boston' },
  { id: 'b6_3', label: 'Đưa ống soi qua hậu môn... qua đại tràng' },
  { id: 'b6_4', label: 'Bơm rửa sạch nhày, dịch... đánh giá tổn thương' },
  { id: 'b6_5', label: 'Xác định tổn thương: vị trí, số lượng, kích thước' },
  { id: 'b6_6', label: 'Hút hơi, dịch, rút ống soi ra trực tràng, soi ngược' },
  { group: 'C. KẾT THÚC & THEO DÕI' },
  { id: 'c7_1', label: 'Khử khuẩn dây soi: Tiệt trùng dây soi theo quy tình' },
  { id: 'c8_1', label: 'Thu dọn vệ sinh & Rác thải đúng quy định' },
  { id: 'c9_1', label: 'Ghi chép hồ sơ đầy đủ diễn biến' },
  { id: 'c10_1', label: 'Theo dõi chỉ số sinh tồn' },
  { id: 'c10_2', label: 'Theo dõi các tai biến và biến chứng' },
  { id: 'c10_3', label: 'Theo dõi phản ứng thuốc tiền mê' },
  { id: 'c10_4', label: 'Dặn dò bệnh nhân/người nhà' },
  { id: 'c10_5', label: 'Bàn giao người bệnh' },
];

const checklistIds = CRITERIA_ENDOSCOPY.filter(c => c.id).map(c => c.id as string);
const TOTAL_CRITERIA = checklistIds.length;

const defaultForm = (userName = ''): Partial<EndoscopyMonitoring> => {
  const base: any = {
...
export const EndoscopyMonitoringModule: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  const { user } = useAuth();
  const [viewMode, setViewMode] = useState<'LIST' | 'FORM' | 'DETAIL'>('LIST');
  const [dataList, setDataList] = useState<EndoscopyMonitoring[]>([]);
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [data, dv] = await Promise.all([fetchEndoscopyMonitoring(), fetchDmDonVi()]);
      setDataList(data || []);
      setDeptList(dv || []);
    } catch (e) {
      console.error(e);
  ...
  const handleSave = async (payload: any) => {
    try {
      if (editingItem?.id) await updateEndoscopyMonitoring(editingItem.id, payload);
      else await addEndoscopyMonitoring(payload);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteEndoscopyMonitoring(id);
      loadData();
    } catch (e: any) { alert('Lỗi: ' + e.message); }
  };
      ...
      <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex bg-white p-1.5 gap-1 rounded-[28px] border border-slate-200 shrink-0 shadow-sm">
            <button className="supervision-tab-button flex items-center gap-2 px-5 py-2.5 rounded-2xl text-[11px] font-bold uppercase tracking-widest transition-all border bg-[#059669] text-white border-[#059669] shadow-lg">
              <Activity size={15} /> Nội soi đại trực tràng
            </button>
          </div>
          {viewMode === 'LIST' && (
            <button onClick={() => { setEditingItem(null); setViewMode('FORM'); }} className="w-full lg:w-auto flex items-center justify-center gap-2 rounded-2xl bg-[#059669] px-6 py-3 text-xs font-black uppercase tracking-wider text-white shadow-xl shadow-green-200 transition-all hover:bg-[#0d6e39] active:scale-95">
        ...
        {loading ? (
          ...
          <div className="animate-in fade-in duration-500">
            {viewMode === 'LIST' ? (
              ...
              <EndoscopyForm item={editingItem} currentUser={user} departmentList={deptList} onSaved={handleSave} onClose={() => setViewMode('LIST')} />
            ) : (
              <EndoscopyDetail item={editingItem} currentUser={user} onClose={() => setViewMode('LIST')} onEdit={() => setViewMode('FORM')} />
            )}
          </div>
        )}
      </div>
...
const EndoscopyForm = ({ item, currentUser, departmentList, onSaved, onClose }: any) => {
    ...
    <div className="bg-white rounded-[40px] shadow-2xl border border-slate-100 overflow-hidden">
      <div className={`p-7 border-b border-slate-50 flex justify-between items-center bg-[#059669]/5`}>
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-main-title font-bold text-slate-800 uppercase leading-tight">{item?.id ? 'Sửa' : 'Thêm'} phiếu nội soi đại trực tràng</h2>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Bảng kiểm quy trình nội soi có gây mê</p>
          </div>
        </div>
...
const EndoscopyDetail = ({ item, currentUser, onClose, onEdit }: any) => {
    ...
    <div className="bg-white rounded-[40px] shadow-2xl p-6 md:p-10 border border-slate-100 relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto space-y-10">
        ...
        <div className="text-center space-y-3">
           <div className="w-16 h-16 rounded-[24px] flex items-center justify-center mx-auto shadow-xl mb-4 bg-emerald-600 text-white"><Activity size={32}/></div>
           <h1 className="text-main-title font-bold text-slate-900 uppercase leading-tight tracking-tight">BẢNG KIỂM QUY TRÌNH NỘI SOI ĐẠI TRỰC TRÀNG CÓ THUỐC GÂY MÊ</h1>
        </div>
