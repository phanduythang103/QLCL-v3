...
export const JCIFallIncidentsModule: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  // Mặc định KHÔNG lọc sẵn theo khoa/người: lọc sẵn khiến danh sách trống khi
  // khoa của user không trùng cách ghi khoa trên phiếu.
...
const FallProcessReport: React.FC<{ data: JCIFallIncident[]; departments: any[] }> = ({ data, departments }) => {
  ...
  const scoped = useMemo(() => {
    ...
    return data.filter(item => {
      const matchPeriod = matchesReportPeriod(item.thoi_gian_nga, reportFilter);
