    ...
    <div className="bg-white rounded-[40px] shadow-2xl border border-slate-100 overflow-hidden">
      ...
      <form onSubmit={(e) => { e.preventDefault(); onSaved(form); }} className="p-7 space-y-7">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 bg-slate-50 p-5 rounded-[28px] border border-slate-100 shadow-inner">
           ...
           <SupervisionScope department={form.khoa_duoc_giam_sat || ''} onDepartmentChange={v => setField('khoa_duoc_giam_sat', v)} userDepartment={currentUser?.department} departments={departmentList} idPrefix="cdmp" label="Khoa" required labelClassName="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2" inputClassName="w-full p-3 rounded-2xl border border-slate-200 text-sm font-bold outline-none" />
           <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2">NVYT / Kíp thủ thuật</label><EmployeeSelect name={form.nguoi_duoc_giam_sat || ''} doiTuong={form.doi_tuong || 'Điều dưỡng'} khoaDonVi={form.khoa_duoc_giam_sat || ''} onChange={({ name, doiTuong }) => setForm((p: any) => ({ ...p, nguoi_duoc_giam_sat: name, doi_tuong: doiTuong }))} required idPrefix="pleural-nguoi-duoc-gs" namePlaceholder="Chọn/nhập tên NVYT" wrapperClassName="flex gap-2" selectClassName="w-28 p-3 rounded-2xl border border-slate-200 text-sm font-bold outline-none bg-white appearance-none" inputClassName="flex-1 p-3 rounded-2xl border border-slate-200 text-sm font-bold outline-none" /></div>
        </div>
