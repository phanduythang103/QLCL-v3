    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="indicator-quick-stats grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          { label: 'Tổng ca', value: records.length, unit: 'ca', icon: FileText, color: 'text-[#059669]', bg: 'bg-[#059669]/10' },
          { label: 'Số phòng mổ', value: new Set(records.map(r => r.phong_mo_so)).size, unit: 'phòng', icon: Layout, color: 'text-[#059669]', bg: 'bg-[#059669]/10' },
        ].map(({ label, value, unit, icon: Icon, color, bg }) => (
          <div key={label} className="indicator-quick-stat-card bg-white rounded-xl p-4 border border-slate-100 shadow-sm flex items-center gap-4 transition-all hover:shadow-md group">
            <div className={`indicator-quick-stat-icon w-10 h-10 ${bg} ${color} rounded-lg flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform duration-300`}>
              <Icon size={20} />
            </div>
            <div className="indicator-quick-stat-body">
              <p className="indicator-quick-stat-label text-[10px] font-bold uppercase text-slate-400 tracking-widest">{label}</p>
              <p className="indicator-quick-stat-value text-xl font-black text-slate-800 tracking-tight">{value} <span className="indicator-quick-stat-unit text-xs font-bold text-slate-400">{unit}</span></p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden mt-6">
        <div className="p-6 border-b border-slate-50 flex items-center justify-between">
          ...
          <button onClick={() => setActiveTab('DANH_SACH')} className="text-[#059669] text-xs font-black uppercase hover:underline">Xem tất cả</button>
        </div>
            ...
            <div key={r.id} className="p-4 space-y-3">
              <div className="flex justify-between items-start">
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm font-black text-[#059669]">{r.total_downtime} ph</p>
                </div>
              </div>
            </div>
          ))}
  ...
  const renderDanhSach = () => (
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex flex-wrap items-center justify-between gap-4">
        <div className="relative flex-1 min-w-[240px]">
          ...
          <input
            ...
            className="w-full pl-11 pr-4 py-3 bg-slate-50 border-none rounded-2xl text-table font-bold focus:ring-2 focus:ring-[#059669]/20 transition-all"
          />
        </div>
        <button
          onClick={handleAdd}
      ...
      <div className="hidden md:block bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
        <table className="w-full text-left">
          <thead>
            <tr className="bg-[#059669] text-white text-[14px] uppercase font-bold tracking-widest border-b border-[#059669]">
          ...
          <tbody className="divide-y divide-slate-50">
            {records.map(r => (
              <tr key={r.id} className="hover:bg-slate-50/50 transition-colors group">
                ...
                <td className="px-6 py-4 text-right">
                  <span className="text-table font-bold text-[#059669] font-mono">{r.total_downtime}</span>
                </td>
                <td className="px-6 py-4 text-right">
      ...
      <div className="md:hidden grid grid-cols-1 gap-4 mt-4">
        {records.map(r => (
          <div key={r.id} className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden flex flex-col hover:shadow-md transition-all active:scale-[0.98]">
            ...
            <div className="px-5 py-4 bg-[#059669]/5 flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                <span className="text-xl font-black text-[#059669] font-mono leading-none">{r.total_downtime} <span className="text-[10px] font-bold text-slate-400 uppercase ml-0.5 tracking-normal">phút</span></span>
                <button
                  onClick={() => {
                    setSelectedRecord(r);
                    setShowDetailModal(true);
                  }}
                  className="bg-[#059669] text-white px-4 py-2 rounded-xl text-[11px] font-bold uppercase shadow-sm active:scale-95 transition-all"
                >
                  Xem
                </button>
              </div>
          ...
          <div className="bg-white rounded-[40px] w-full max-w-4xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleSave} className="p-8 space-y-8 max-h-[85vh] overflow-y-auto custom-scrollbar">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="space-y-1.5 relative group/select">
                  ...
                  <div className="relative">
                    <div
                      tabIndex={0}
                      className="w-full px-4 py-3 bg-slate-100 rounded-2xl text-[10px] font-black min-h-[42px] flex flex-wrap gap-1 items-center cursor-pointer hover:bg-slate-200 transition-colors border border-transparent focus:ring-2 focus:ring-[#059669]/20"
                    >
                        ...
                        <div className="flex flex-1 gap-4 md:gap-0">
                          ...
                          <div className="w-24 md:w-32 md:px-4 md:py-2 border-l border-slate-100 md:border-none md:pl-0">
                            ...
                            <input
                              ...
                              className="w-full bg-transparent border-none p-0 text-xs font-black text-[#059669] focus:ring-0"
                            />
                          </div>
                        </div>
              ...
              <div className="pt-6 border-t border-slate-100 flex items-center justify-between bg-[#059669]/10 -mx-8 -mb-8 p-8 mt-auto rounded-b-[40px]">
                <div>
                  <p className="text-[10px] font-black uppercase text-[#059669]/60 tracking-widest">Tổng thời gian chết</p>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-[40px] w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 space-y-4 overflow-y-auto custom-scrollbar bg-slate-50/30">
                  ...
                  <div key={d.stt} className="bg-white p-4 rounded-[28px] border border-slate-100 shadow-sm space-y-3">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-start gap-3">
                        <div className="w-8 h-8 bg-[#059669]/10 rounded-xl flex items-center justify-center text-[#059669] shrink-0">
                        ...
                        <div className="space-y-0.5">
                          <p className="text-[8px] font-black uppercase text-[#059669]/60 tracking-widest">Mốc theo dõi</p>
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <p className="text-sm font-black text-[#059669] font-mono">{d.time_min} <span className="text-[10px] font-bold text-slate-400 uppercase tracking-normal">phút</span></p>
                      </div>
                    </div>
                    {d.reason && (
              ...
              <div className="p-5 bg-[#059669] rounded-[32px] text-white flex justify-between items-center shadow-lg shadow-[#059669]/20">
                <div className="flex items-center gap-3">
