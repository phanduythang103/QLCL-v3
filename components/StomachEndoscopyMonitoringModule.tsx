...
import {
  fetchStomachEndoscopyMonitoring, addStomachEndoscopyMonitoring, updateStomachEndoscopyMonitoring,
  deleteStomachEndoscopyMonitoring
} from '../readStomachEndoscopyMonitoring';
...
export const CRITERIA_STOMACH_ENDOSCOPY = [
  { group: 'A. GIAI ĐOẠN CHUẨN BỊ' },
  { id: 'a1_1', label: 'Tên, tuổi, địa chỉ, phòng bệnh' },
  { id: 'a1_2', label: 'Chẩn đoán' },
  { id: 'a1_3', label: 'Chỉ định kỹ thuật: Nội soi TQ-DD-TT có thuốc tiền mê' },
  { id: 'a1_4', label: 'Đã nhịn ăn tối thiểu 6 giờ' },
  { id: 'a1_5', label: 'Đã được nghe giải thích và đồng ý làm thủ thuật (kí cam kết)' },
  { id: 'a1_6', label: 'Phiếu khai thác tiền sử dị ứng' },
  { id: 'a2_1', label: '01 Bác sĩ chuyên khoa nội soi tiêu hóa' },
  { id: 'a2_2', label: '02 Điều dưỡng hoặc kỹ thuật viên phụ' },
  { id: 'a2_3', label: 'Trang phục phòng hộ đầy đủ, rửa tay đúng quy định' },
  { id: 'a3_1', label: 'Cam kết chi trả nếu kỹ thuật, thuốc ngoài BHYT' },
  { id: 'a3_2', label: 'Các xét nghiệm cần có để thực hiện kỹ thuật' },
  { id: 'a3_3', label: 'Máy hút' },
  { id: 'a3_4', label: 'Canuyn ngậm miệng' },
  { id: 'a3_5', label: 'Hệ thống bơm rửa' },
  { id: 'a3_6', label: 'Chất bôi trơn đầu máy soi' },
  { id: 'a3_7', label: 'Găng, gạc, bơm tiêm 5ml và 20 ml' },
  { id: 'a3_8', label: 'Kiểm tra thuốc tiền mê, VTYT' },
  { id: 'a4_1', label: 'Kiểm tra phiếu chỉ định, xét nghiệm' },
  { group: 'B. CÁC BƯỚC TIẾN HÀNH' },
  { id: 'b5_1', label: 'Đo mạch, huyết áp' },
  { id: 'b5_2', label: 'Đặt dịch truyền TM, thở oxy, mắc monitor theo dõi' },
  { id: 'b5_3', label: 'Tư thế bệnh nhân (nằm nghiêng phải...)' },
  { id: 'b5_4', label: 'Đặt ngáng miệng, hướng dẫn bệnh nhân ngậm chặt' },
  { id: 'b5_5', label: 'Tiêm thuốc tiền mê' },
  { id: 'b6_1', label: 'Đưa ống soi qua miệng vào thực quản... quan sát' },
  { id: 'b6_2', label: 'Xác định vị trí tổn thương và lấy mẫu bệnh phẩm' },
  { id: 'b7_1', label: 'Sinh thiết làm test Hp, nhuộm màu, can thiệp...' },
  { id: 'b7_2', label: 'Hút hơi, dịch, rút ống soi, vệ sinh miệng' },
  { id: 'b7_3', label: 'Theo dõi tình trạng bệnh nhân trong suốt quá trình nội soi' },
  { group: 'C. KẾT THÚC & THEO DÕI' },
  { id: 'c8_1', label: 'Khử khuẩn dây soi theo quy trình' },
  { id: 'c9_1', label: 'Thu dọn vệ sinh & Rác thải đúng quy định' },
  { id: 'c10_1', label: 'Ghi chép hồ sơ đầy đủ diễn biến' },
  { id: 'c11_1', label: 'Theo dõi chỉ số sinh tồn' },
  { id: 'c11_2', label: 'Theo dõi các tai biến và biến chứng của nội soi' },
  { id: 'c11_3', label: 'Theo dõi phản ứng thuốc tiền mê' },
  { id: 'c11_4', label: 'Dặn dò bệnh nhân/người nhà' },
  { id: 'c11_5', label: 'Bàn giao người bệnh' },
];

const checklistIds = CRITERIA_STOMACH_ENDOSCOPY.filter(c => c.id).map(c => c.id as string);
const TOTAL_CRITERIA = checklistIds.length; // 37

const defaultForm = (userName = ''): Partial<StomachEndoscopyMonitoring> => {
  const base: any = {
...
export const StomachEndoscopyMonitoringModule: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  const { user } = useAuth();
  const [viewMode, setViewMode] = useState<'LIST' | 'FORM' | 'DETAIL'>('LIST');
  const [dataList, setDataList] = useState<StomachEndoscopyMonitoring[]>([]);
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [data, dv] = await Promise.all([fetchStomachEndoscopyMonitoring(), fetchDmDonVi()]);
      setDataList(data || []);
      setDeptList(dv || []);
    } catch (e) {
      console.error(e);
  ...
  const handleSave = async (payload: any) => {
    try {
      if (editingItem?.id) await updateStomachEndoscopyMonitoring(editingItem.id, payload);
      else await addStomachEndoscopyMonitoring(payload);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteStomachEndoscopyMonitoring(id);
      loadData();
    } catch (e: any) { alert('Lỗi: ' + e.message); }
  };
      ...
      <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex bg-white p-1.5 gap-1 rounded-[28px] border border-slate-200 shrink-0 shadow-sm">
            <button className="supervision-tab-button flex items-center gap-2 px-5 py-2.5 rounded-2xl text-[11px] font-bold uppercase tracking-widest transition-all border bg-fuchsia-600 text-white border-fuchsia-600 shadow-lg">
              <Activity size={15} /> Nội soi TQDD có gây mê
            </button>
          </div>
          {viewMode === 'LIST' && (
            <button onClick={() => { setEditingItem(null); setViewMode('FORM'); }} className="w-full lg:w-auto flex items-center justify-center gap-2 rounded-2xl bg-fuchsia-600 px-6 py-3 text-xs font-black uppercase tracking-wider text-white shadow-xl shadow-fuchsia-200 transition-all hover:bg-fuchsia-700 active:scale-95">
        ...
        {loading ? (
          <div className="flex justify-center items-center h-64"><Loader2 className="animate-spin text-fuchsia-600" size={32} /></div>
        ) : (
          <div className="animate-in fade-in duration-500">
            {viewMode === 'LIST' ? (
              ...
              <StomachEndoscopyForm item={editingItem} currentUser={user} departmentList={deptList} onSaved={handleSave} onClose={() => setViewMode('LIST')} />
            ) : (
              <StomachEndoscopyDetail item={editingItem} currentUser={user} onClose={() => setViewMode('LIST')} onEdit={() => setViewMode('FORM')} />
            )}
          </div>
        )}
      </div>
...
const StomachEndoscopyForm = ({ item, currentUser, departmentList, onSaved, onClose }: any) => {
  ...
  return (
    <div className="bg-white rounded-[40px] shadow-2xl border border-slate-100 overflow-hidden">
      <div className={`p-7 border-b border-slate-50 flex justify-between items-center bg-fuchsia-600/5`}>
        <div className="flex items-center gap-4">
          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white shadow-lg bg-fuchsia-600`}>
            <Activity size={22} />
          </div>
          <div>
            <h2 className="text-main-title font-bold text-slate-800 uppercase leading-tight">{item?.id ? 'Sửa' : 'Thêm'} phiếu nội soi TQ-DD-TT</h2>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Bảng kiểm quy trình nội soi có tiền mê</p>
          </div>
        </div>
      ...
      <form onSubmit={(e) => { e.preventDefault(); onSaved(form); }} className="p-7 space-y-7">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 bg-slate-50 p-5 rounded-[28px] border border-slate-100 shadow-inner">
           <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2">Ngày giám sát</label><input type="date" value={form.ngay_giam_sat} onChange={e => setField('ngay_giam_sat', e.target.value)} required className="w-full p-3 rounded-2xl border border-slate-200 text-sm font-bold focus:ring-4 focus:ring-fuchsia-500/10 outline-none" /></div>
           ...
           <SupervisionScope department={form.khoa_phong || ''} onDepartmentChange={v => setField('khoa_phong', v)} userDepartment={currentUser?.department} departments={departmentList} idPrefix="stomach-endoscopy" label="Khoa/Phòng" required labelClassName="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2" inputClassName="w-full p-3 rounded-2xl border border-slate-200 text-sm font-bold outline-none" />
           <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2">Người được giám sát</label><EmployeeSelect name={form.nguoi_duoc_giam_sat || ''} doiTuong={form.doi_tuong || 'Điều dưỡng'} khoaDonVi={form.khoa_phong || ''} onChange={({ name, doiTuong }) => setForm((p: any) => ({ ...p, nguoi_duoc_giam_sat: name, doi_tuong: doiTuong }))} required idPrefix="stomach-endoscopy-nguoi-duoc-gs" namePlaceholder="Chọn/nhập họ tên" wrapperClassName="flex gap-2" selectClassName="w-28 p-3 rounded-2xl border border-slate-200 text-sm font-bold outline-none bg-white appearance-none" inputClassName="flex-1 p-3 rounded-2xl border border-slate-200 text-sm font-bold outline-none" /></div>
        </div>

        <div className="bg-white border border-slate-200 rounded-[32px] overflow-hidden shadow-sm">
           <div className="p-4 text-white text-[10px] font-black uppercase tracking-widest bg-fuchsia-600">Nội dung kiểm tra (Các bước thực hiện)</div>
           <div className="divide-y divide-slate-50">
             {CRITERIA_STOMACH_ENDOSCOPY.map((c: any, i: number) => {
               if (c.group) {
                 return (
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 pt-6 border-t border-slate-100">
          ...
          <div className="flex gap-3 w-full md:w-auto">
             ...
             <button type="submit" className={`flex-1 md:flex-none px-12 py-3.5 text-white rounded-2xl text-[11px] font-black uppercase shadow-xl transition-all bg-fuchsia-600 hover:bg-fuchsia-700 shadow-fuchsia-100`}>Lưu phiếu</button>
          </div>
        </div>
      </form>
    </div>
...
const StomachEndoscopyDetail = ({ item, currentUser, onClose, onEdit }: any) => {
  ...
  return (
    <div className="bg-white rounded-[40px] shadow-2xl p-6 md:p-10 border border-slate-100 relative overflow-hidden">
      <div className={`absolute top-0 left-0 w-full h-2 bg-fuchsia-600`}/>
      <div className="max-w-4xl mx-auto space-y-10">
        <div className="flex justify-between items-center no-print">
          ...
          <div className="flex gap-3">
             ...
             {isAdmin && <button onClick={onEdit} className={`px-6 py-3 text-white rounded-2xl text-[11px] font-black uppercase shadow-lg bg-fuchsia-600 shadow-fuchsia-100`}>Sửa dữ liệu</button>}
          </div>
        </div>

        <div className="text-center space-y-3">
           <div className="w-16 h-16 rounded-[24px] flex items-center justify-center mx-auto shadow-xl mb-4 bg-fuchsia-600 text-white"><Activity size={32}/></div>
           <h1 className="text-main-title font-bold text-slate-900 uppercase leading-tight tracking-tight">BẢNG KIỂM QUY TRÌNH NỘI SOI TQ-DD-TT CÓ THUỐC TIỀN MÊ</h1>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse border-4 border-slate-900 text-[12pt]">
            <thead className="bg-fuchsia-600 text-white font-bold uppercase text-table">
