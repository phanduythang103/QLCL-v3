    ...
    <div className="bg-white rounded-[40px] shadow-2xl border border-slate-100 overflow-hidden">
      ...
      <form onSubmit={handleSubmit} className="p-7 space-y-7">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-slate-50 p-5 rounded-[28px] border border-slate-100">
           ...
           <div className="space-y-1"><SupervisionScope department={form.don_vi_duoc_kiem_tra} onDepartmentChange={v => setField('don_vi_duoc_kiem_tra', v)} userDepartment={currentUser?.department} departments={deptList} idPrefix="cd-truc" label="Đơn vị được giám sát" required containerClassName="space-y-1" labelClassName="text-label font-bold text-slate-400 uppercase tracking-widest pl-2" inputClassName="w-full p-2.5 rounded-xl border border-slate-200 text-sm font-bold outline-none" /></div>
           ...
           <div className="space-y-1"><label className="text-label font-bold text-slate-400 uppercase tracking-widest pl-2">Nhân viên được giám sát</label><EmployeeSelect name={form.nguoi_duoc_giam_sat || ''} doiTuong={form.doi_tuong || 'Điều dưỡng'} khoaDonVi={form.don_vi_duoc_kiem_tra || ''} onChange={({ name, doiTuong }) => setForm((p: any) => ({ ...p, nguoi_duoc_giam_sat: name, doi_tuong: doiTuong }))} required idPrefix="cdtruc-nguoi-duoc-gs" namePlaceholder="Chọn/nhập họ tên" wrapperClassName="flex gap-2" selectClassName="w-24 p-2.5 rounded-xl border border-slate-200 text-sm font-bold outline-none bg-white appearance-none" inputClassName="flex-1 p-2.5 rounded-xl border border-slate-200 text-sm font-bold outline-none" /></div>
        </div>
