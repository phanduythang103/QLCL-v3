...
import {
  fetchBronchoscopyMonitoring, addBronchoscopyMonitoring, updateBronchoscopyMonitoring,
  deleteBronchoscopyMonitoring
} from '../readBronchoscopyMonitoring';
...
export const CRITERIA_BRONCHOSCOPY = [
  { group: 'A. GIAI ĐOẠN CHUẨN BỊ' },
  { id: 'a1_1', label: 'Chuẩn bị người bệnh: Đúng người, khám trước thủ thuật, nằm ngửa' },
  { id: 'a2_1', label: 'Nhân viên y tế (Kíp 3 người: 1BS + 2 ĐD)' },
  { id: 'a2_2', label: 'Trang phục phòng hộ đầy đủ, rửa tay, găng vô khuẩn/hữu khuẩn' },
  { id: 'a3_1', label: 'Chuẩn bị thuốc: tê Lidocain, cấp cứu' },
  { id: 'a3_2', label: 'Chuẩn bị dụng cụ: dàn máy, theo dõi, kìm sinh thiết, formol' },
  { id: 'a4_1', label: 'Hồ sơ bệnh án: Đủ XN, chỉ định, giấy cam kết' },
  { group: 'B. CÁC BƯỚC TIẾN HÀNH' },
  { id: 'b5_1', label: 'Bước 1: Đưa ống qua mũi/miệng, gây tê thanh môn, vào khí quản' },
  { id: 'b6_1', label: 'Bước 2: Đưa ống đúng trình tự, gây tê bổ sung, quan sát, mô tả tổn thương' },
  { id: 'b7_1', label: 'Bước 3: Mở kìm sinh thiết (4-6 mảnh), bảo quản formol, xử trí cầm máu' },
  { id: 'b8_1', label: 'Bước 4: Đánh giá toàn trạng, bàn giao người bệnh, bàn giao bệnh phẩm' },
  { group: 'C. KẾT THÚC & THEO DÕI' },
  { id: 'c9_1', label: 'Ghi chép đầy đủ diễn biến, KQ vào hồ sơ, chỉ định XN' },
  { id: 'c10_1', label: 'Theo dõi đánh giá tình trạng NB, dặn nhịn ăn uống 2 tiếng' },
  { id: 'c11_1', label: 'Thu dọn vệ sinh & Rác thải đúng quy định (TT20)' },
];

const checklistIds = CRITERIA_BRONCHOSCOPY.filter(c => c.id).map(c => c.id as string);
const TOTAL_CRITERIA = checklistIds.length; // 13

const defaultForm = (userName = ''): Partial<BronchoscopyMonitoring> => {
  const base: any = {
...
export const BronchoscopyMonitoringModule: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  const { user } = useAuth();
  const [viewMode, setViewMode] = useState<'LIST' | 'FORM' | 'DETAIL'>('LIST');
  const [dataList, setDataList] = useState<BronchoscopyMonitoring[]>([]);
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [data, dv] = await Promise.all([fetchBronchoscopyMonitoring(), fetchDmDonVi()]);
      setDataList(data || []);
      setDeptList(dv || []);
    } catch (e) {
      console.error(e);
  ...
  const handleSave = async (payload: any) => {
    try {
      if (editingItem?.id) await updateBronchoscopyMonitoring(editingItem.id, payload);
      else await addBronchoscopyMonitoring(payload);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deleteBronchoscopyMonitoring(id);
      loadData();
    } catch (e: any) { alert('Lỗi: ' + e.message); }
  };
      ...
      <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex bg-white p-1.5 gap-1 rounded-[28px] border border-slate-200 shrink-0 shadow-sm">
            <button className="supervision-tab-button flex items-center gap-2 px-5 py-2.5 rounded-2xl text-[11px] font-bold uppercase tracking-widest transition-all border bg-orange-500 text-white border-orange-500 shadow-lg">
              <Activity size={15} /> Nội soi phế quản sinh thiết
            </button>
          </div>
          {viewMode === 'LIST' && (
            <button onClick={() => { setEditingItem(null); setViewMode('FORM'); }} className="w-full lg:w-auto flex items-center justify-center gap-2 rounded-2xl bg-orange-500 px-6 py-3 text-xs font-black uppercase tracking-wider text-white shadow-xl shadow-orange-200 transition-all hover:bg-orange-600 active:scale-95">
        ...
        {loading ? (
          ...
          <div className="animate-in fade-in duration-500">
            {viewMode === 'LIST' ? (
              ...
              <BronchoscopyForm item={editingItem} currentUser={user} departmentList={deptList} onSaved={handleSave} onClose={() => setViewMode('LIST')} />
            ) : (
              <BronchoscopyDetail item={editingItem} currentUser={user} onClose={() => setViewMode('LIST')} onEdit={() => setViewMode('FORM')} />
            )}
          </div>
        )}
      </div>
...
const BronchoscopyForm = ({ item, currentUser, departmentList, onSaved, onClose }: any) => {
  ...
  return (
    <div className="bg-white rounded-[40px] shadow-2xl border border-slate-100 overflow-hidden">
      <div className={`p-7 border-b border-slate-50 flex justify-between items-center bg-orange-500/5`}>
        <div className="flex items-center gap-4">
          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white shadow-lg bg-orange-500`}>
            <Activity size={22} />
          </div>
          <div>
            <h2 className="text-main-title font-bold text-slate-800 uppercase leading-tight">{item?.id ? 'Sửa' : 'Thêm'} phiếu Nội soi phế quản sinh thiết</h2>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Quy trình kỹ thuật nội soi phế quản</p>
          </div>
        </div>
      ...
      <form onSubmit={(e) => { e.preventDefault(); onSaved(form); }} className="p-7 space-y-7">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 bg-slate-50 p-5 rounded-[28px] border border-slate-100 shadow-inner">
           <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2">Ngày giám sát</label><input type="date" value={form.ngay_giam_sat} onChange={e => setField('ngay_giam_sat', e.target.value)} required className="w-full p-3 rounded-2xl border border-slate-200 text-sm font-bold focus:ring-4 focus:ring-orange-500/10 outline-none" /></div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 pt-6 border-t border-slate-100">
          ...
          <div className="flex gap-3 w-full md:w-auto">
             ...
             <button type="submit" className={`flex-1 md:flex-none px-12 py-3.5 text-white rounded-2xl text-[11px] font-black uppercase shadow-xl transition-all bg-orange-500 hover:bg-orange-600 shadow-orange-100`}>Lưu phiếu</button>
          </div>
        </div>
      </form>
    </div>
...
const BronchoscopyDetail = ({ item, currentUser, onClose, onEdit }: any) => {
      ...
      <div className="max-w-4xl mx-auto space-y-10">
        <div className="flex justify-between items-center no-print">
          ...
          <div className="flex gap-3">
             ...
             {isAdmin && <button onClick={onEdit} className={`px-6 py-3 text-white rounded-2xl text-[11px] font-black uppercase shadow-lg bg-orange-500 shadow-orange-100`}>Sửa dữ liệu</button>}
          </div>
        </div>

        <div className="text-center space-y-3">
           <div className="w-16 h-16 rounded-[24px] flex items-center justify-center mx-auto shadow-xl mb-4 bg-orange-500 text-white"><Activity size={32}/></div>
           <h1 className="text-main-title font-bold text-slate-900 uppercase leading-tight tracking-tight">GIÁM SÁT TUÂN THỦ NỘI SOI PHẾ QUẢN SINH THIẾT</h1>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse border-4 border-slate-900 text-[12pt]">
            <thead className="bg-orange-500 text-white font-bold uppercase text-table">
