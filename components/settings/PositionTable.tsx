...
import { fetchDmChucVu, addDmChucVu, updateDmChucVu, deleteDmChucVu } from '../../readDmChucVu';
...
export default function PositionTable() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const data = await fetchDmChucVu();
      console.log('DmChucVu from Supabase:', data);
      setItems(data || []);
      setError(null);
    } catch (err: any) {
      console.error('Error fetching dm_chuc_vu:', err);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await updateDmChucVu(editingId, form);
        setMessage('Cập nhật thành công!');
      } else {
        await addDmChucVu(form);
      ...
      {showForm && (
        <div className="bg-white border border-slate-200 rounded-lg p-4 shadow-sm">
          <div className="flex justify-between items-center mb-4">
            <h4 className="text-label font-black text-black uppercase">{editingId ? 'Sửa chức vụ' : 'Thêm chức vụ mới'}</h4>
            <button onClick={resetForm} className="text-slate-400 hover:text-slate-600"><X size={20} /></button>
          </div>
          <form onSubmit={handleSubmit} className="flex gap-4 items-end">
            <div className="flex-1">
              <input required placeholder="Tên chức vụ *" value={form.ten_chuc_vu} onChange={e => setForm(f => ({ ...f, ten_chuc_vu: e.target.value }))} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-input font-bold text-black focus:outline-none focus:border-green-500" />
            </div>
            ...
            {items.length === 0 ? (
              ...
              items.map((item, idx) => (
                <tr key={item.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-black font-black text-table uppercase">{item.ten_chuc_vu}</td>
