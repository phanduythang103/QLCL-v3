      ...
      {showForm && (
        <div className="bg-white border border-slate-200 rounded-lg p-4 shadow-sm">
          <div className="flex justify-between items-center mb-4">
            <h4 className="font-semibold text-slate-800">{editingId ? 'Sửa chức vụ' : 'Thêm chức vụ mới'}</h4>
            <button onClick={resetForm} className="text-slate-400 hover:text-slate-600"><X size={20} /></button>
          </div>
          <form onSubmit={handleSubmit} className="flex gap-4 items-end">
            <div className="flex-1">
              <input required placeholder="Tên chức vụ *" value={form.ten_chuc_vu} onChange={e => setForm(f => ({ ...f, ten_chuc_vu: e.target.value }))} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary-500" />
            </div>
            ...
            {items.length === 0 ? (
              ...
              items.map((item, idx) => (
                <tr key={item.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-slate-700">{item.ten_chuc_vu}</td>
