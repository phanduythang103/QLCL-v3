            ...
            <div key={item.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">#{idx + 1}</p>
                  <h3 className="mt-1 line-clamp-2 text-sm font-black uppercase leading-snug text-slate-800">{item.nd_giam_sat}</h3>
                </div>
                <span className={`shrink-0 px-2.5 py-1 rounded-lg text-[10px] font-black uppercase ${item.trang_thai === 'Đang thực hiện' ? 'bg-blue-100 text-blue-700' :
                  item.trang_thai === 'Hoàn thành' ? 'bg-green-100 text-green-700' :
                    item.trang_thai === 'Quá hạn' ? 'bg-red-100 text-red-700' :
                      'bg-slate-100 text-slate-700'
                }`}>
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">
                <div className="rounded-xl bg-slate-50 p-3">
                  <p className="text-[9px] font-black uppercase text-slate-400">Từ ngày</p>
                  <p className="mt-1 text-xs font-black text-slate-800">{formatDate(item.tu_ngay)}</p>
                </div>
                <div className="rounded-xl bg-slate-50 p-3">
                  <p className="text-[9px] font-black uppercase text-slate-400">Đến ngày</p>
                  <p className="mt-1 text-xs font-black text-slate-800">{formatDate(item.den_ngay)}</p>
                </div>
              </div>
              <div className="mt-3 space-y-2 text-[11px] font-bold text-slate-500">
                <p className="line-clamp-2"><span className="font-black text-slate-700">NV GS:</span> {item.nhan_vien_gs || '-'}</p>
                <p className="line-clamp-2"><span className="font-black text-slate-700">ĐV GS:</span> {item.dv_duoc_gs || '-'}</p>
              </div>
              <div className="mt-4 flex justify-end gap-2">
                <button onClick={() => handleEdit(item)} className="rounded-xl border border-primary-100 bg-primary-50 p-2 text-primary-600"><Edit2 size={16} /></button>
                <button onClick={() => handleDelete(item.id)} className="rounded-xl border border-red-100 bg-red-50 p-2 text-red-600"><Trash2 size={16} /></button>
              </div>
            </div>
          ))
        )}
