      ...
      <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-slate-500 text-xs font-bold uppercase tracking-widest mt-1">Quản lý và thống kê tuân thủ vệ sinh tay</p>
          </div>
          <button 
            onClick={() => { setEditingItem(null); setIsReadOnly(false); setIsFormOpen(true); }}
            className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-2xl text-sm font-black uppercase tracking-wider transition-all shadow-xl shadow-indigo-200 active:scale-95 w-full md:w-auto justify-center"
          >
            <Plus size={20} /> Thêm giám sát
          </button>
        </div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center gap-4">
          ...
          {/* Filter Bar */}
          <div className="flex-1 bg-white rounded-[32px] border border-slate-200 shadow-sm overflow-hidden">
            <button 
              onClick={() => setShowFilters(!showFilters)}
              className="w-full p-3 md:p-4 flex items-center justify-between hover:bg-slate-50 transition-all text-left"
            >
              <div className="flex items-center gap-2">
                <Filter size={14} className="text-indigo-500" />
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Bộ lọc dữ liệu</span>
              </div>
              <div className={`lg:hidden p-1 rounded-lg bg-slate-100 text-slate-400 transition-transform duration-300 ${showFilters ? 'rotate-180' : ''}`}>
                 <Plus size={12} className={showFilters ? 'rotate-45' : ''} />
              </div>
            </button>
            
            <div className={`${showFilters ? 'block' : 'hidden'} lg:block p-4 lg:p-4 pt-0 lg:pt-0 border-t lg:border-t-0 border-slate-100 animate-in slide-in-from-top duration-300`}>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                <div className="space-y-1">
                  <label className="text-[8px] font-black text-slate-400 uppercase tracking-wider ml-1">Thời gian</label>
                  <select 
                    value={filterConfig.preset}
                    onChange={e => setFilterConfig({...filterConfig, preset: e.target.value})}
                    className="w-full px-3 py-2 bg-slate-50 border-none rounded-xl text-[10px] font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all appearance-none cursor-pointer"
                  >
                ...
                {filterConfig.preset === 'TUY_CHON' && (
                  <>
                    <div className="space-y-1 animate-in slide-in-from-left duration-300">
                      <label className="text-[8px] font-black text-slate-400 uppercase tracking-wider ml-1">Từ ngày</label>
                      <input 
                        type="date" 
                        value={filterConfig.fromDate}
                        onChange={e => setFilterConfig({...filterConfig, fromDate: e.target.value})}
                        className="w-full px-3 py-2 bg-slate-50 border-none rounded-xl text-[10px] font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all" 
                      />
                    </div>
                    <div className="space-y-1 animate-in slide-in-from-left duration-300">
                      <label className="text-[8px] font-black text-slate-400 uppercase tracking-wider ml-1">Đến ngày</label>
                      <input 
                        type="date" 
                ...
                <div className="space-y-1">
                  <label className="text-[8px] font-black text-slate-400 uppercase tracking-wider ml-1">Khoa giám sát</label>
                  <select 
                ...
                <div className="flex items-end">
                  <button 
                    ...
                    className="flex items-center justify-center gap-2 w-full px-4 py-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all text-[9px] font-black uppercase tracking-widest border border-dashed border-slate-200 hover:border-rose-200"
                  >
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tổng lượt giám sát</p>
            <h3 className="text-2xl font-black text-slate-800 tracking-tight">{stats.totalMonitors}</h3>
          </div>
        </div>
        <div className="bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tỷ lệ tuân thủ</p>
            <h3 className="text-2xl font-black text-emerald-600 tracking-tight">{stats.complianceRate.toFixed(1)}%</h3>
          </div>
        </div>
        <div className="bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Đúng kỹ thuật</p>
            <h3 className="text-2xl font-black text-blue-600 tracking-tight">{stats.techniqueRate.toFixed(1)}%</h3>
          </div>
        </div>
      </div>
